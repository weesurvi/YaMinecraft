...
class Items(Entity):
	def __init__(self,texture,model="cube",position=(0,1,3),scale=(0.003,0.2,0.2)):
		super().__init__(self,texture=texture,model="cube",position=(0,1,3),scale=(0.003,0.2,0.2))
	def update(self):
		if distance(First, self) <= 2 and "empty" in icon_list:
			empty_space=0
			for i in range(0,9):
...
class Block(Entity):
	...
	def input(self,key):
		...
		if key == "9":
			invs_sel=8
		if self.hovered:
			if key == "left mouse down" and self.typ != "bedrock":
				destroy(self)
		...
		for j in range(stz,stz+16):
			for k in range(sty,0):
				if random.randint(0,101)<=undground_perc:
					...
					elif adf >= 886 and adf <= 950:
						Block(model="cube",texture="diorite.png",position=(i,k,j),collider='box',typ="diorite")
					elif adf >= 951 and adf <= 985:
						Block(model="cube",texture="iron_ore.png",position=(i,k,j),collider='box',typ="iron_ore")
					elif adf >= 986 and adf <= 998:
						Block(model="cube",texture="coal.png",position=(i,k,j),collider='box',typ="coal")
					else:
						Block(model="cube",texture="diamond.png",position=(i,k,j),collider='box',typ="diamond")
