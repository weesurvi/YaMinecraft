...
if days == "day":
	...
	moon=Entity(model="cube",texture="moon.png",scale=(30,30,30),position=(0,300,0))
	window.color=color.rgb(40,40,40)
	for i in range(-400,400,10):
		for j in range(-400,400,10):
			if random.randint(0,101)>=98:
				if random.randint(0,101) >= 20:
					Entity(model="cube",color=color.white,position=(i,400,j))
				else:
					Entity(model="cube",color=color.orange,position=(i,400,j))
